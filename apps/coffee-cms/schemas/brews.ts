...
export default  {
  ...
  fields: [
    ...
    },
    {
      ...
      options: {
        list: [
          {title: 'Reddi-wip Nitro Creamer', value: 'reddi-wip nitro creamer'},
    ...
    {
      name: 'name',
      description: 'Publish record with "Brewed With" and "Brewed On" filled out first before trying to generate a name',
      type: 'string',
      ...
      options: {
        ...
        reduceQueryResult: (query) => {
          ...
          if(query && query.records) {
            const foundRecord = query.records.find(({_id}) => _id === query._id)
            if(foundRecord) {
